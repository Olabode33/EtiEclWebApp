...
@Component({
  selector: 'app-createEdit-wholesaleEcl',
  templateUrl: './createEdit-wholesaleEcl.component.html',
  styleUrls: ['./createEdit-wholesaleEcl.component.css'],
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()]
})
export class CreateEditWholesaleEclComponent extends AppComponentBase implements OnInit {
    ...
    ngOnInit() {
        ...
        this.wholesaleECL.reportingDate = moment().endOf('month');
    }
    ...
    saveWholesaleEcl(): void {
        this.message.confirm(
            this.l('SubmitOrSaveAsDraft'),
            (isConfirmed) => {
                if (isConfirmed) {
                    this.wholesaleECL.status = EclStatusEnum.Submitted;
                } else {
                    this.wholesaleECL.status = EclStatusEnum.Draft;
                }
                this._wholesaleEcLsServiceProxy.createOrEdit(this.wholesaleECL)
                        .subscribe(() => {
                            this.navigateToWorkSpace();
                            this.message.success('WholesaleEclSuccessfullyCreated');
                        });
            }
        );
    }
