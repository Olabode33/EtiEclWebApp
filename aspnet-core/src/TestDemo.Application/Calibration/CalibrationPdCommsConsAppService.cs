...
namespace TestDemo.Calibration
{
    ...
    public class CalibrationPdCommsConsAppService : TestDemoAppServiceBase, ICalibrationsAppService
    {
        private readonly IRepository<CalibrationPdCommsCons, Guid> _calibrationRepository;
        private readonly IRepository<CalibrationPdCommsConsApproval, Guid> _calibrationApprovalRepository;
        private readonly IRepository<CalibrationInputPdCommsCon> _calibrationInputRepository;
        private readonly IRepository<CalibrationResultPdCommsCons> _calibrationResultRepository;
        ...
        public CalibrationPdCommsConsAppService(
            IRepository<CalibrationPdCommsCons, Guid> calibrationRepository, 
            IRepository<User, long> lookup_userRepository,
            IRepository<CalibrationPdCommsConsApproval, Guid> calibrationApprovalRepository,
            IRepository<CalibrationInputPdCommsCon> calibrationInputRepository,
            IRepository<OrganizationUnit, long> organizationUnitRepository,
            IRepository<CalibrationResultPdCommsCons> calibrationResultRepository,
        ...
        public async Task<CalibrationInputSummaryDto<InputPdCommsConsDto>> GetInputSummary(EntityDto<Guid> input)
        {
        ...
        public async Task<CalibrationInputSummaryDto<InputPdCommsConsDto>> GetHistorySummary(EntityDto<Guid> input)
        {
        ...
        public async Task<GetAllResultPdCommsConsDto> GetResult(EntityDto<Guid> input)
        {
            var commsCons = await _calibrationResultRepository.GetAll().Where(x => x.CalibrationId == input.Id)
                                                              .Select(x => ObjectMapper.Map<ResultNewPdCommsConsDto>(x))
                                                              .OrderBy(x => x.Month)
                                                              .ToListAsync();

            return new GetAllResultPdCommsConsDto
            {
        ...
        protected virtual async Task<Guid> Create(CreateOrEditCalibrationRunDto input)
        {
            ...
            if (userSubsidiaries.Count > 0)
            {
                ...
                Guid id = await _calibrationRepository.InsertAndGetIdAsync(new CalibrationPdCommsCons()
                {
        ...
        public async Task UpdateCalibrationResultCommCons(List<ResultNewPdCommsConsDto> input)
        {
            var result = ObjectMapper.Map<List<CalibrationResultPdCommsCons>>(input);
            ...
            {
                await _calibrationResultRepository.UpdateAsync(item);
            }

            await _calibrationApprovalRepository.InsertAsync(new CalibrationPdCommsConsApproval
            {
        ...
        public async Task ApplyToEcl(EntityDto<Guid> input)
        {
            ...
            if (calibration.Status == CalibrationStatusEnum.Completed)
            {
                ...
                await _backgroundJobManager.EnqueueAsync<SaveHistoricPdCommsConsDataJob, ImportCalibrationDataFromExcelJobArgs>(new ImportCalibrationDataFromExcelJobArgs
                {
        ...
        public async Task Erase(EntityDto<Guid> input)
        {
            ...
            if (calibration.Status != CalibrationStatusEnum.AppliedToEcl)
            {
                ...
                {
                    EraseType = TrackTypeEnum.CalibratePdCommsCons,
                    GuidId = input.Id
                });
            }
            else
        ...
        public async Task SendSubmittedEmail(Guid calibrationId)
        {
            ...
            var link = baseUrl + "/app/main/calibration/pdcommscons/view/" + calibrationId;
